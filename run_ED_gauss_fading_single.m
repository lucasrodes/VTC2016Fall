function [Texp_h0_var,Texp_h0_pdf,Texp_h1_var,Texp_h1_pdf,Tth_var,...
    ...
    run_ED_gauss_fading_single(y,w,N)
...
%ROC curve, for a Gauss distributed TX signal in a Rayleigh Fading channel 
...
% S = 1e7;
% hx = importdata('signal_gauss.mat');
% w = sqrt(0.65/2)*(randn(1,S)+1i*randn(1,S));%importdata('H0_1.mat');
% y = hx+w;
% 10*log10(var(y)/var(w)-1)

% y = read_complex_binary('path_to_file'); % Write path of the GNU Radio measurment H1 file
% w = importdata('path_to_file'); % Write path of the .mat file H0
