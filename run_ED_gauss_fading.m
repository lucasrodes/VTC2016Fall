function run_ED_gauss_fading(RXtech, N, SNR_input)
%RUN_ED_CS_FADING(RXtech, N, SNR) - This function focuses on a PU gauss 
%distributed signal with an Rayleigh fading channel.
...
% Load files
w1 = importdata([files_dir '/Gauss/Rayleigh Fading/H0_1.mat']);
if SNR_input>=0
    y1 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad' ...
        num2str(SNR_input) 'SNR_1.mat']);
else
    y1 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad_m'...
...
if sum(RXtech(2:5))>0 % If any multiple antenna (L=2) technique is selected
    w2 = importdata([files_dir '/Gauss/Rayleigh Fading/H0_2.mat']);
    if SNR_input>=0
        y2 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad_' num2str(SNR_input)...
            'SNR_2.mat']);
    else
        y2 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad_m' ...
    ...
    if (RXtech(3)+RXtech(5))>0 % If any multiple antenna (L=4) technique is selected
        ...
        if SNR_input>=0
            y3 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad_' num2str(SNR_input)...
                'SNR_3.mat']);
            y4 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad_' num2str(SNR_input)...
                'SNR_4.mat']);
        else
            y3 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad_m' ...
                num2str(abs(SNR_input)) 'SNR_3.mat']);
            y4 = importdata([files_dir '/Gauss/Rayleigh Fading/H1_fad_m' ...
...
if RXtech(1) 
    ...
    run_ED_gauss_fading_single(y1,w1,N); 

    % Plot ROC curve
    figure(1);
    plot(Pfa_th,Pd_th,'color',g1,'linewidth',4,'displayname',...
        'Theoretical results (Normal approximation) [Rayleigh Fading]');  
        hold on; 
    plot(Pfa_exp,Pd_exp,'color',g2,'linewidth',2,'displayname',...
        'Experimental results [Rayleigh Fading]');
    ...
    legend('H0 Theoretical (Normal Approximation)', ...
        ...
        'H1 Experimental'); 
    title(['Rayleigh Fading Channel (SNR = ' num2str(SNR) 'dB ; N = '...
...
if RXtech(2) 
    ...
    [Texp_h0_var,Texp_h0_pdf,Texp_h1_var,Texp_h1_pdf,Tth_var,...
    Tth_h0_pdf,Tth_h1_pdf,Pd_exp,Pfa_exp,Pd_th,Pfa_th] = ...
    run_ED_gauss_fading_slc2(y1,w1,y2,w2,N);
    ...
    plot(Pfa_exp,Pd_exp,'color',b2,'linewidth',2,'displayname',...
        'Experimental results [Rayleigh Fading + SLC, L=2]'); 
    ...
    title(['Rayleigh fading Channel SLC, L=2 (SNR = ' num2str(SNR) 'dB ; N ='...
         num2str(N) ')'],'fontsize',16);
    xlabel('Threshold \lambda','fontsize',16);
    ylabel('PDF of the test statistic T(Y)','fontsize',16);
    grid on; hold off; 
...
if RXtech(3) 
    ...
    [Texp_h0_var,Texp_h0_pdf,Texp_h1_var,Texp_h1_pdf,Tth_var,...
    Tth_h0_pdf,Tth_h1_pdf,Pd_exp,Pfa_exp,Pd_th,Pfa_th] = ...
    run_ED_gauss_fading_slc4(y1,w1,y2,w2,y3,w3,y4,w4,N);
    
    % Plot ROC curve
    figure(1);
    plot(Pfa_th,Pd_th,'color',bb1,'linewidth',4,'displayname',...
        'Theoretical results (Normal approximation) [Rayleigh Fading + SLC, L=4]'); 
        hold on;
    plot(Pfa_exp,Pd_exp,'color',bb2,'linewidth',2,'displayname',...
        'Experimental results [Rayleigh Fading + SLC, L=4]'); 
    ...
    legend( 'H0 Theoretical (Normal Approximation)','H0 Experimental'...
        ,'H1 Theoretical (Normal Approximation)','H1 Experimental'); 
    title(['Rayleigh Fading Channel SLC, L=4 (SNR = ' num2str(SNR) 'dB ; N ='...
         num2str(N) ')'],'fontsize',16);
    xlabel('Threshold \lambda','fontsize',16);
    ylabel('PDF of the test statistic T(Y)','fontsize',16);
    grid on; hold off; 
...
if RXtech(4) 
    ...
    plot(Pfa_exp,Pd_exp,'color',r2,'linewidth',2,'displayname',...
        'Experimental results [Rayleigh Fading + SLS]'); 
    ...
    legend( 'H0 Theoretical (Normal Approximation)','H0 Experimental'...
        ,'H1 Theoretical (Normal Approximation)','H1 Experimental'); 
    title(['Rayleigh Fading Channel SLS (SNR = ' num2str(SNR) 'dB ; N ='...
         num2str(N) ')'],'fontsize',16);
    xlabel('Threshold \lambda','fontsize',16);
    ylabel('PDF of the test statistic T(Y)','fontsize',16);
    grid on; hold off; 
...
% Square-Law Selector (SLS) [L=4]
if RXtech(5) 
    % Run script to obtain all parameters
    [Texp_h0_var,Texp_h0_pdf,Texp_h1_var,Texp_h1_pdf,Tth_var,...
    Tth_h0_pdf,Tth_h1_pdf,Pd_exp,Pfa_exp,Pd_th,Pfa_th] = ...
    run_ED_gauss_fading_sls4(y1,w1,y2,w2,y3,w3,y4,w4,N);
    
    % Plot ROC curve
    figure(1);
    plot(Pfa_th,Pd_th,'color',rr1,'linewidth',4,'displayname',...
        'Theoretical results (Normal approximation) [Rayleigh Fading + SLS, L=4]'); 
        hold on;
    plot(Pfa_exp,Pd_exp,'color',rr2,'linewidth',2,'displayname',...
        'Experimental results [Rayleigh Fading + SLS, L=4]'); 
    ...
    legend( 'H0 Theoretical (Normal Approximation)','H0 Experimental'...
        ,'H1 Theoretical (Normal Approximation)','H1 Experimental'); 
    title(['Rayleigh Fading Channel SLS, L=4 (SNR = ' num2str(SNR) 'dB ; N ='...
         num2str(N) ')'],'fontsize',16);
    xlabel('Threshold \lambda','fontsize',16);
    ylabel('PDF of the test statistic T(Y)','fontsize',16);
    grid on; hold off; 
