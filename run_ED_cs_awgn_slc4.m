...
%RUN_ED_CS_AWGN_SLC(y1, w1, y2, w2, N) - This function compares theoretical 
%and empirical results of the test statistic probability, i.e. its PDF and 
%the ROC curve, for a Complex Sinusoid TX signal in an AWGN Channel and 
% using a Square-Law Combiner (SLC) at the SU.
...
Tth_h0_pdf = pdf('Normal',Tth_var,a1*sigma_w1^2+a2*sigma_w2^2+a3*sigma_w3^2+a4*sigma_w4^2,sqrt(...
    (a1^2*sigma_w1^4+a2^2*sigma_w2^4+a3^2*sigma_w3^4+a4^2*sigma_w4^4)/N));

% Theoretical PDF of the Test Statistic under H1
Tth_h1_pdf = pdf('Normal',Tth_var,a1*Prx1+a2*Prx2+a3*Prx3+a4*Prx4,sqrt((a1^2*(2*...
    Prx1*sigma_w1^2-sigma_w1^4)+a2^2*(2*Prx2*sigma_w2^2-sigma_w2^4)...
    +a3^2*(2*Prx3*sigma_w3^2-sigma_w3^4)...
    +a4^2*(2*Prx4*sigma_w4^2-sigma_w4^4))/N));
...
for Tk=Tth_var
    ...
    Pd_th(cont) = qfunc((Tk-(a1*Prx1+a2*Prx2+a3*Prx3+a4*Prx4))./...
        sqrt((a1^2*(2*Prx1*sigma_w1^2-sigma_w1^4)+a2^2*(2*Prx2*...
        sigma_w2^2-sigma_w2^4)+a3^2*(2*Prx3*sigma_w3^2-sigma_w3^4)...
        +a4^2*(2*Prx4*sigma_w4^2-sigma_w4^4))/N));
    % Pfa for threshold Tk (Experimental + Theoretical)
    Pfa_exp(cont) = sum(T_h0 > Tk)/length(T_h0);
    Pfa_th(cont) = qfunc((Tk-(a1*sigma_w1^2+a2*sigma_w2^2++a3*sigma_w3^2++a4*sigma_w4^2))/sqrt((a1^2*...
        sigma_w1^4+a2^2*sigma_w2^4+a3^2*sigma_w3^4+a4^2*sigma_w4^4)/N));
