function run_ED_cs_fading(RXtech, N, SNR_input)
%RUN_ED_CS_FADING(RXtech, N, SNR) - This function focuses on a PU complex 
%sinusoid signal with an Rayleigh fading channel.
...
% Load files
w1 = importdata([files_dir '/Complex Sinusoid/AWGN/H0_1.mat']);
if SNR_input>=0
    y1 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad' ...
        num2str(SNR_input) 'SNR_1.mat']);
else
    y1 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad_m'...
...
if sum(RXtech(2:5))>0 % If any multiple antenna (L=2) technique is selected
    w2 = importdata([files_dir '/Complex Sinusoid/AWGN/H0_2.mat']);
    if SNR_input>=0
        y2 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad_' num2str(SNR_input)...
            'SNR_2.mat']);
    else
        y2 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad_m' ...
    ...
    if (RXtech(3)+RXtech(5))>0 % If any multiple antenna (L=4) technique is selected
        w3 = importdata([files_dir '/Complex Sinusoid/AWGN/H0_3.mat']);
        w4 = importdata([files_dir '/Complex Sinusoid/AWGN/H0_4.mat']);
        if SNR_input>=0
            y3 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad_' num2str(SNR_input)...
                'SNR_3.mat']);
            y4 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad_' num2str(SNR_input)...
                'SNR_4.mat']);
        else
            y3 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad_m' ...
                num2str(abs(SNR_input)) 'SNR_3.mat']);
            y4 = importdata([files_dir '/Complex Sinusoid/Rayleigh Fading/H1_fad_m' ...
...
if RXtech(1) 
    ...
    run_ED_cs_fading_single(y1,w1,N); 
...
if RXtech(2) 
    ...
    [Texp_h0_var,Texp_h0_pdf,Texp_h1_var,Texp_h1_pdf,Tth_var,...
    Tth_h0_pdf,Tth_h1_pdf,Pd_exp,Pfa_exp,Pd_th,Pfa_th] = ...
    run_ED_cs_fading_slc2(y1,w1,y2,w2,N);
    ...
    plot(Pfa_exp,Pd_exp,'color',b2,'linewidth',2,'displayname',...
        'Experimental results [Rayleigh daing + SLC, L=2]'); 
...
if RXtech(4) 
    ...
    [Texp_h0_var,Texp_h0_pdf,Texp_h1_var,Texp_h1_pdf,Tth_var,...
    Tth_h0_pdf,Tth_h1_pdf,Pd_exp,Pfa_exp,Pd_th,Pfa_th] = ...
    run_ED_cs_fading_sls2(y1,w1,y2,w2,N);
    ...
    plot(Pfa_exp,Pd_exp,'color',r2,'linewidth',2,'displayname',...
        'Experimental results [Rayleigh daing + SLS, L=2]'); 
    ...
    legend( 'H0 Theoretical (Normal Approximation)','H0 Experimental'...
        ,'H1 Theoretical (Normal Approximation)','H1 Experimental','fontsize',16); 
    title(['Rayleigh Fading Channel SLS, L=2 (SNR = ' num2str(SNR) 'dB ; N ='...
         num2str(N) ')'],'fontsize',16);
    xlabel('Threshold \lambda','fontsize',16);
    ylabel('PDF of the test statistic T(Y)','fontsize',16);
    grid on; hold off; 
...
if RXtech(5) 
    ...
    [Texp_h0_var,Texp_h0_pdf,Texp_h1_var,Texp_h1_pdf,Tth_var,...
    Tth_h0_pdf,Tth_h1_pdf,Pd_exp,Pfa_exp,Pd_th,Pfa_th] = ...
    run_ED_cs_fading_sls4(y1,w1,y2,w2,y3,w3,y4,w4,N);
